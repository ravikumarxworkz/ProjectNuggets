...
<body>
			...
			<form id="forgotPasswordForm" action="forgotPassword" method="post">
				...
				<div class="form-group mt-3" style="display: none;"
					...
					<label for="newPassword">New Password:</label> <input
						...
						name="newPassword" required> <input type="checkbox"
						id="showPasswordCheckbox"
						onchange="togglePasswordVisibility('newPassword')"> <label
						for="showPasswordCheckbox">Show Password</label> <span
				...
				<div class="form-group" style="display: none;"
					...
					<label for="confirmPassword">Confirm Password:</label> <input
						...
						onchange="togglePasswordVisibility('confirmPassword')"> <label
						for="showConfirmPasswordCheckbox">Show Password</label> <span
	...
	<script>
		...
		function validateEmail() {
			...
			$
					.ajax({
						url : 'http://localhost:8080/WomenSecurityApps/reemail?email='
								+ userEmail,
						type : 'GET',
						success : function(response) {
							if (response.trim() === 'exists') {
								document.getElementById('emailError').innerHTML = ' ';
								document.getElementById('emailSuccess').innerHTML = 'OTP Sent To Your EamilId.';
								document.getElementById('otpSection').style.display = 'block';
								document.getElementById("checkEmail").classList
										.add("btn-success");
								document.getElementById("checkEmail").disabled = true;
							} else {
								document.getElementById('emailError').innerHTML = 'Enter Registred EmailId.';
							}
						},
		...
		function validateOTP() {
			...
			if (otpValue != null && otpValue != "") {
				...
				$
						.ajax({
							url : 'http://localhost:8080/WomenSecurityApps/validateOTP',
							type : 'POST',
							data : {
								otp : otpValue,
								email : userEmail
							...
							success : function(response) {
								if (response.trim() === 'valid') {
									...
									document.getElementById('otpsucess').innerHTML = 'OTP Verified Successfully!.';
